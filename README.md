...
- 三级流水线由三个单线程有界队列串接（LLM→TTS→V2F 各一 worker）：单线程消费保证音频帧严格有序，乱序会直接破坏音画同步；有界队列容量 1000 提供背压，LLM 入口队列满则整请求熔断（计数并在流末下发错误标记），避免上游无限堆积内存。
- 音画同步是核心难点：TTS 输出 22050Hz 与 Audio2Face 输入 16kHz 采样率不一致，流水线对音频切片做线性重采样适配输入域；但 V2F 模型按固定样本数切帧，输入域降采样后同段音频的帧数按比例缩减，直接推理会破坏音画时长对齐，因此推理后按"音频时长 × 30FPS"补齐或截断表情帧，保证表情序列与音频严格等长。
...
- AvatarStreamingComponent 维护 gRPC 流，内置端侧熔断：端侧缓冲积压超阈值即强制清空截断流，作为回复长度失控的最后防线。
